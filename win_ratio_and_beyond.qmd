...
- **GPC within stratum and aggregate** [@dong2017; @dong2023]
...
$$
\hat w^{(a, 1-a)} = (n_an_{1-a})^{-1} \sum_{i=1}^{n_a}\sum_{j=1}^{n_{1-a}} Y^\a_i (1 - Y^\b_j) = \hat p_a (1 - \hat p_{1-a})
$$
...
-   **Alternative hypothesis**
    - Death and hospitalization times: *stochastically larger* in treatment than in control [@Luo2015; @Mao2019]
...
- **Trial data** [@gillmore:2024]
    ...
    - **Primary HCE:** Death → CV hospitalization → NT-proBNP → 6-min walk distance  
...
- **Trial data** [@tavares:2024]
    ...
    - **Intervention:** Dapagliflozin 10 mg daily vs standard care alone (up to 14 days)  
    - **Primary HCE:** Hospital mortality → initiation of KRT → ICU length of stay (28 days)  
</div>
...
$$
    ...
    - Elicit effect size $\log(\text{WR})$ and $p_{\text{tie}}$ from component-wise
...
- **Specify time horizon** $\tau$
    - Imagine all subjects followed up to $\tau$ (1 yr, 3 yrs, etc.)
...
-   **Restricted mean time** in favor of treatment
    ...
    $$
    -   $w_{a, 1-a}(\tau)$: average *time* subject in group $a$ wins over subject in group $1-a$ up to time $\tau$
...
- **Some caveats** [@butler:2024]
    -  WR ignores ties
        -  Win relative to *loss* rather than *"not win"*
        -  Some consider this to overstate effect size 
    -  Inclusion of low-priority biomarker
        -   Surrogate measures drives treatment effect
