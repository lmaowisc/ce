...
For a two-tiered composite of death and a nonfatal event, the restricted win/loss probability is defined by
\begin{align}
w_{a, 1-a}(\tau) &= \pr\{D^\b < \min(D^\a, \tau)\} \\
&\quad + \pr\{\min(D^\t, D^\c) > \tau, T^\b < \min(T^\a, \tau)\},
\end{align}
with the restricted win ratio defined as $\text{WR}(\tau) = w_{1,0}(\tau) / w_{0,1}(\tau)$. Estimation methods include inverse probability of censoring weighting (IPCW) and multiple imputation (MI), as implemented in the `WINS` package.
